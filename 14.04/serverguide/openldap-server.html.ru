...
<p class="para">Установка slapd создаст работающую конфигурацию. В частности, она создаст экземпляр базы данных, которую вы можете использовать для хранения своих данных. Однако суффикс (или базовый DN) этого экземпляра будет определён из доменного имени localhost. Если вы хотите использовать что-то другое, отредактируйте <span class="file filename">/etc/hosts</span> и замените доменное имя на подходящее. Например, если вам нужен суффикс <span class="em emphasis">dc=example,dc=com</span>, то ваш файл должен иметь подобную строку:</p>
<div class="code"><pre class="contents ">127.0.1.1       hostname.example.com	hostname
</pre></div>
<p class="para">Вы можете отменить изменения после установки пакета.</p>
<div class="note" title="Примечание"><div class="inner"><div class="region"><div class="contents">
...
<li class="list itemizedlist">
			<p class="para"><span class="em emphasis">olcBackend={0}hdb,cn=config</span>: тип хранилища 'hdb' заднего плана</p>
			</li>
<li class="list itemizedlist">
...
<li class="list itemizedlist">
			<p class="para"><span class="em emphasis">olcDatabase={1}hdb,cn=config</span>: экземпляр вашей базы данных (dc=examle,dc=com)</p>
			</li>
</ul></div>
...
<div class="list itemizedlist"><ul class="list itemizedlist">
<li class="list itemizedlist">
	<p class="para">Используйте <span class="app application">ldapmodify</span>для добавления индекса (атрибут DbIndex) для вашей <span class="app application">{1}hdb,cn=config</span> базы (dc=example,dc=com). Создайте файл с названием <span class="file filename">uid_index.ldif</span> следующего содержания:</p>
...
/var/lib/ldap/** rwk,
		...
		<p class="para">Создаём каталог, устанавливаем файл настроек базы данных и перезагружаем профиль apparmor:</p>
...
<li class="steps">
		<p class="para">Добавляем новый контент и, поскольку изменили apparmor, перезапускаем сервис:</p>
...
<div class="steps"><div class="inner"><ol class="steps">
<li class="steps">
		<p class="para">Установим программное обеспечение как указано в <a class="xref" href="openldap-server.html#openldap-server-installation" title="Установка">Установка</a>. Убедитесь, что база slapd-config аналогична базе Поставщика. Особенно проверьте, что одинаковы схемы и суффикс базы.</p>
	</li>
<li class="steps">
...
<p class="para">Самый первый ACL очень важен:</p>
...
<p class="para">Этот составной ACL (их два) предписывает следующее:</p>
<div class="list itemizedlist"><ul class="list itemizedlist">
<li class="list itemizedlist">
	<p class="para">Анонимный 'auth' доступ обеспечивается к атрибуту <span class="em emphasis">userPassword</span> для осуществления изначального соединения. Возможно потребуется counter-intuitively для 'by anonymous auth' даже когда анонимный доступ к DIT не требуется. Как только удаленное соединение установлено, требуется аутентификация (см. следующий пункт).</p>
	</li>
<li class="list itemizedlist">
	<p class="para">Должна пройти аутентификация, поскольку все пользователи имеют доступ на чтение (вследствие 'by self write') к атрибуту <span class="em emphasis">userPassword</span>.</p>
	</li>
<li class="list itemizedlist">
	<p class="para">Атрибут <span class="em emphasis">userPassword</span> не доступен для всех других пользователей за исключением rootDN, который имеет полный доступ.</p>
	</li>
<li class="list itemizedlist">
	<p class="para">Для того чтобы пользователи могли менять собственные пароли, используя <span class="cmd command">passwd</span> или иные утилиты, атрибут <span class="em emphasis">shadowLastChange</span> должен быть доступен как только пользователь авторизовался.</p>
	</li>
</ul></div>
<p class="para">Поиск по этому DIT может быть проведен анонимно из-за 'by * read' в данном ACL:</p>
...
<p class="para">Как указывалось ранее, для базы slapd-config  не создаётся никаких административных пользователей. Однако существует идентификация SASL, которая обеспечивает полный доступ к ней. Она подобна суперпользователю для localhost (root/sudo). Вот она:</p>
...
<p class="para">Сужаем права на владение и доступ:</p>
...
<p class="para">Перезапустите OpenLDAP:</p>
<div class="screen"><pre class="contents "><span class="cmd command">sudo service slapd restart</span>
</pre></div>
<p class="para">Проверьте журналы вашего хоста (/var/log/syslog), чтобы убедиться, что сервер запущен правильно.</p>
</div></div>
</div></div>
