...
<a href="../../14.04" class="trail">Ubuntu 14.04</a> » <a class="trail" href="index.html" title="Guía de Ubuntu Server">Guía de Ubuntu Server</a> » <a class="trail" href="vpn.html" title="VPN">VPN</a> » </div></div>
<div id="cwt-content" class="clearfix content-area"><div id="page">
<div id="content">
<div class="links nextlinks">
<a class="nextlinks-prev" href="vpn.html" title="VPN">Anterior</a><a class="nextlinks-next" href="other-useful-applications.html" title="Otras aplicaciones útiles">Siguiente</a>
</div>
...
<li class="links"><a class="xref" href="openvpn.html#openvpn-server-installation" title="Instalación del servidor">Instalación del servidor</a></li>
<li class="links"><a class="xref" href="openvpn.html#openvpn-pki-setup" title="Configuración de la infraestructura de clave pública">Configuración de la infraestructura de clave pública</a></li>
<li class="links"><a class="xref" href="openvpn.html#openvpn-simple-server-configuration" title="Configuración sencilla de servidor">Configuración sencilla de servidor</a></li>
<li class="links"><a class="xref" href="openvpn.html#openvpn-simple-client-configuration" title="Configuración sencilla de cliente">Configuración sencilla de cliente</a></li>
...
<li class="links"><a class="xref" href="openvpn.html#openvpn-advanced-config" title="Configuración avanzada">Configuración avanzada</a></li>
<li class="links"><a class="xref" href="openvpn.html#openvpn-client-implementations" title="Implementaciones de software en el cliente">Implementaciones de software en el cliente</a></li>
<li class="links"><a class="xref" href="openvpn.html#openvpn-references" title="Referencias">Referencias</a></li>
</ul></div>
<div class="sect2 sect" id="openvpn-server-installation"><div class="inner">
<div class="hgroup"><h2 class="title">Instalación del servidor</h2></div>
<div class="region"><div class="contents">
<p class="para">Para instalar <span class="app application">openvpn</span> introduzca en una terminal:</p>
...
<div class="hgroup"><h2 class="title">Configuración de la infraestructura de clave pública</h2></div>
<div class="region">
<div class="contents">
<p class="para">El primer paso es construir una configuración OpenVPN para establecer una PKI (infraestructura de clave pública). La PKI consiste en:</p>
<div class="list itemizedlist"><ul class="list itemizedlist">
<li class="list itemizedlist"><p class="para">un certificado separado (también conocido como clave pública) y clave privada para el servidor y cada cliente y</p></li>
<li class="list itemizedlist"><p class="para">un certificado de Autoridad Certificadora (AC) maestra y clave que se usa para firmar cada uno de los certificados de servidores y clientes.</p></li>
</ul></div>
<p class="para">OpenVPN permite autenticación bidireccional basada en certificados, lo que significa que el cliente debe autenticar el certificado del servidor y el servidor debe autenticar el certificado del cliente antes que se establezca la confianza mutua.</p>
<p class="para">Ambos, servidor y cliente, autenticarán a la otra parte verificando primero que el certificado presentado fue firmado por la autoridad certificadora maestra (AC), y después comprobando la información de la cabecera del certificado ya autenticado, tal coo el nombre del certificado común o el tipo de certificado (cliente o servidor).</p>
</div>
<div class="sect3 sect" id="openvpn-ca-setup"><div class="inner">
<div class="hgroup"><h3 class="title">Configuración de la autoridad de certificación</h3></div>
<div class="region"><div class="contents">
<p class="para">Para configurar su propia Autoridad Certificadora (AC) y generar certificados y claves para un servidor de OpenVPN y varios clientes copie primero el directorio <span class="file filename">easy-rsa</span> a <span class="file filename">/etc/openvpn</span>. Esto asegurará que los cambios a los scripts no se pierdan cuando se actualice el paquete. Desde un terminal cambie al usuario «root» y:</p>
...
<p class="para">Ahora edite <span class="file filename">/etc/openvpn/easy-rsa/vars</span> ajustando lo siguiente a su entorno:</p>
<div class="code"><pre class="contents ">export KEY_COUNTRY="ES"
...
<p class="para">Para generar el certificado y la clave maestra de la Autoridad Certificadora (AC), introduzca lo siguiente:</p>
...
<div class="hgroup"><h3 class="title">Certificados de servidor</h3></div>
<div class="region"><div class="contents">
<p class="para">Ahora, generaremos un certificado y clave privada para el servidor:</p>
<div class="screen"><pre class="contents "><span class="cmd command">./build-key-server myservername</span>
</pre></div>
<p class="para">Como en los pasos previos, la mayoría de los parámetros pueden usar valores predeterminados. Otras dos consultan necesitan de respuestas positivas, "¿Firmar el certificado? [s/n]" y "1 de 1 peticiones de certificados certificadas, ¿confirmar? [s/n]".</p>
<p class="para">Los parámetros Diffie Hellman se deben generar para el servidor OpenVPN:</p>
<div class="screen"><pre class="contents "><span class="cmd command">./build-dh</span>
</pre></div>
<p class="para">Todos los certificados y claves han sido generados en el subdirectorio keys/. Es una práctica común copiarlos a /etc/openvpn/:</p>
...
<div class="hgroup"><h3 class="title">Certificados de cliente</h3></div>
<div class="region"><div class="contents">
<p class="para">El cliente VPN también necesitará de un certificado para autenticarse él mismo al servidor. Normalmente creará un certificado diferente para cada cliente. Para crear el certificado, escriba lo siguiente en el terminal como usuario «root»:</p>
<div class="screen"><pre class="contents "><span class="cmd command">cd /etc/openvpn/easy-rsa/</span>
<span class="cmd command">source vars</span>
<span class="cmd command">./build-key client1</span>
</pre></div>
<p class="para">Copie los siguientes archivos al cliente usando un método seguro:</p>
<div class="list itemizedlist"><ul class="list itemizedlist">
...
<p class="para">Puesto que los certificados y claves de los clientes solo se necesitan en el equipo cliente, debería eliminarlos del servidor.</p>
</div></div>
</div></div>
</div>
</div></div>
<div class="sect2 sect" id="openvpn-simple-server-configuration"><div class="inner">
<div class="hgroup"><h2 class="title">Configuración sencilla de servidor</h2></div>
<div class="region"><div class="contents">
<p class="para">Junto con la instalación de <span class="app application">OpenVPN</span> se incluyen estos archivos de configuración de ejemplo (y muchos más si lo comprueba):</p>
...
<p class="para">Empiece copiando y desempaquetando server.conf.gz a /etc/openvpn/server.conf.</p>
...
<p class="para">Edite el archivo <span class="file filename">/etc/sysctl.conf</span> y active (elimine el «#») la línea siguiente para permitir el desvío de IP.</p>
<div class="code"><pre class="contents ">#net.ipv4.ip_forward=1
</pre></div>
<p class="para">A continuación, vuelva a cargar sysctl.</p>
...
<p class="para">Ahora compruebe si OpenVPN ha creado una iterfaz tun0:</p>
...
<div class="hgroup"><h2 class="title">Configuración sencilla de cliente</h2></div>
<div class="region"><div class="contents">
<p class="para">
...
<p class="para">Ahora copie el archivo de configuración de ejemplo client.conf a /etc/openvpn/.</p>
...
<p class="para">Revise sus rutas:</p>
...
<div class="list itemizedlist"><ul class="list itemizedlist">
<li class="list itemizedlist"><p class="para">Compruebe su syslog, ej. grep -i vpn /var/log/syslog</p></li>
...
<div class="hgroup"><h2 class="title">Configuración avanzada</h2></div>
<div class="region">
...
<p class="para">Si se activa, esta directiva configurará todos los clientes para que redirijan sus puertas de enlace de red predeterminadas a través de la VPN; produciendo así que todo el tráfico de IP, como la navegación web y las búsquedas de DNS, pase por la VPN (para que esto funcione adecuadamente, puede que se necesite usar NAT en la interfaz TUN/TAP que apunta a la internet).</p>
...
<p class="para">Fuerce los servidores DNS en el cliente.</p>
<div class="code"><pre class="contents ">push "dhcp-option DNS 10.0.0.2"
push "dhcp-option DNS 10.1.0.2"
</pre></div>
<p class="para">Permitir la comunicación con el cliente.</p>
<div class="code"><pre class="contents ">client-to-client
</pre></div>
<p class="para">Activar compresión en el enlace VPN.</p>
...
<p class="para">Es una buena idea reducir los privilegios del demonio OpenVPN después de la inicialización.</p>
...
<div class="note" title="Nota"><div class="inner"><div class="region"><div class="contents"> <p class="para">
...
<div class="region"><div class="contents">
<p class="para">Asegúrese de tener instalado el paquete bridge-utils:</p>
...
<div class="region"><div class="contents">
<p class="para">Edite <span class="file filename">/etc/openvpn/server.conf</span> cambiando las siguientes opciones a:</p>
...
<p class="para">Luego, conviértalo en ejecutable:</p>
<div class="screen"><pre class="contents "><span class="cmd command">sudo chmod 755 /etc/openvpn/up.sh</span>
</pre></div>
<p class="para">Después de configurado el servidor, reinicie <span class="app application">openvpn</span> introduciendo:</p>
...
<div class="hgroup"><h4 class="title">Configuración del cliente</h4></div>
<div class="region"><div class="contents">
<p class="para">Primero instale <span class="app application">openvpn</span> en el cliente:</p>
<div class="screen"><pre class="contents "><span class="cmd command">sudo apt-get install openvpn</span>
</pre></div>
<p class="para">Entonces, con el servidor configurado y el certificado del cliente copiado en el directorio <span class="file filename">/etc/openvpn/</span>, creado una configuración cliente copiando el ejemplo. En una terminal en el equipo cliente, ejecute:</p>
<div class="screen"><pre class="contents "><span class="cmd command">sudo cp /usr/share/doc/openvpn/examples/sample-config-files/client.conf /etc/openvpn</span>
</pre></div>
<p class="para">Ahora edite <span class="file filename">/etc/openvpn/client.conf</span> cambiando las siguientes opciones:</p>
...
<p class="para">Finalmente reinicie <span class="app application">openvpn</span>:</p>
<div class="screen"><pre class="contents "><span class="cmd command">sudo service openvpn restart</span>
</pre></div>
<p class="para">Ahora debe ser capaz de conectar a la LAN remota a través de VPN.</p>
</div></div>
</div></div>
</div>
</div></div>
...
<div class="hgroup"><h2 class="title">Implementaciones de software en el cliente</h2></div>
<div class="region">
<div class="contents"></div>
<div class="sect3 sect" id="openvpn-client-networkmanager"><div class="inner">
<div class="hgroup"><h3 class="title">Interfaz del gestor de red de Linux para OpenVPN</h3></div>
<div class="region"><div class="contents">
<p class="para">
...
<p class="para">Debe reiniciar network-manager para que detecte los paquetes recién instalados:</p>
...
<div class="hgroup"><h3 class="title">OpenVPN con interfaz para Mac OS X: Tunnelblick</h3></div>
<div class="region"><div class="contents">
<p class="para">
...
<div class="hgroup"><h3 class="title">OpenVPN con interfaz para Win 7</h3></div>
<div class="region"><div class="contents">
<p class="para">
...
<div class="hgroup"><h3 class="title">OpenVPN para OpenWRT</h3></div>
<div class="region"><div class="contents">
<p class="para">
...
<p class="para">Acceda a su router OpenWRT e instale OpenVPN:</p>
...
<p class="para">Reinicie OpenVPN:</p>
...
<li class="list itemizedlist">
          <p class="para">Vea el sitio web <a href="http://openvpn.net/" class="ulink" title="http://openvpn.net/">OpenVPN</a> para más información.</p>
        </li>
<li class="list itemizedlist">
          <p class="para">
...
<li class="list itemizedlist">
          <p class="para">También, Pakt's <a href="http://www.packtpub.com/openvpn/book" class="ulink" title="http://www.packtpub.com/openvpn/book">OpenVPN: Construyendo e Integrando Redes Privadas Virtuales</a> es un buen recurso.</p>
        </li>
</ul></div></div></div>
</div></div>
</div>
