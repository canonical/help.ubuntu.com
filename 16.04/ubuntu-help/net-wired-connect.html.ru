...
<div class="region">
<div class="contents">
<p class="p">Всё, что потребуется, чтобы настроить большинство проводных сетевых соединений — это подсоединить сетевой шнур. Значок сети на верхней панели должен несколько секунд повращаться, а затем принять вид «разъёма», когда соединение установлено.</p>
<p class="p">Если этого не произошло, первым делом проверьте, подключён ли сетевой шнур. Один его конец должен быть вставлен в прямоугольный разъём Ethernet на вашем компьютере, второй — подключён к коммутатору, маршрутизатору, разъёму на стене или чему-то подобному (зависит от того, как устроена ваша сеть). Иногда световой индикатор рядом с разъёмом Ethernet показывает, что кабель подключён и активен.</p>
<div class="note" title="Примечание"><div class="inner"><div class="region"><div class="contents"><p class="p">Нельзя просто подключить один компьютер к другому сетевым кабелем (по крайней мере, без некоторых дополнительных настроек). Чтобы соединить два компьютера, нужно подключить каждый из них к сетевому концентратору, маршрутизатору или коммутатору.</p></div></div></div></div>
<p class="p">Если соединение всё ещё не установлено, возможно, ваша сеть не поддерживает автоматическую настройку (DHCP). В таком случае, придётся <span class="link"><a href="net-manual.html" title="Ручная настройка сетевых параметров">настроить её вручную</a></span>.</p>
</div>
