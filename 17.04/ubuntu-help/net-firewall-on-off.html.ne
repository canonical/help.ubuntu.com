...
<p class="p">Each program that provides services uses a specific <span class="em">network port</span>. To enable access to that program's services, you may need to allow access to its assigned port on the firewall. To allow connections on port 53, enter <span class="cmd">sudo ufw allow 53</span> in a terminal. To block port 53, enter <span class="cmd">sudo ufw deny 53</span>.</p>
