...
<div class="region"><div class="contents"><p class="p">Для включения сетевого экрана введите <span class="cmd">sudo ufw enable</span> в терминале. Для отключения введите <span class="cmd">sudo ufw disable</span>.</p></div></div>
</div></div>
...
<p class="p">Каждая программа, предоставляющая сетевые службы, использует определённый <span class="em">сетевой порт</span>. Чтобы разрешить доступ к службам этой программы, необходимо разрешить доступ к используемому ею порту в межсетевом экране. Так, чтобы разрешить соединения через порт 53, введите в терминале <span class="cmd">sudo ufw allow 53</span>. Чтобы заблокировать порт 53, введите <span class="cmd">sudo ufw deny 53</span>.</p>
