...
<div id="content">
<div class="hgroup pagewide"><h1 class="title"><span class="title">Проблемы с устройствами для чтения карт памяти</span></h1></div>
<div class="region">
...
<li class="steps"><p class="p">Откройте приложение <span class="app">Файлы</span> из меню <span class="gui"><a href="shell-introduction.html.ru#activities" title="Режим обзора">Обзор</a></span>. Отображается ли подключенная карта на левой боковой панели? Иногда карта появляется в этом списке, , но её файловая система не смонтирована; нажмите на название карты в списке, чтобы подключить карту. (Если боковая панель не видна, нажмите <span class="key"><kbd>F9</kbd></span> или нажмите в <span class="gui">Файлах</span> на кнопку меню в верхней панели и выберите <span class="gui">Показать боковую панель</span>.)</p></li>
<li class="steps"><p class="p">Если карта памяти не отображается в боковой панели, нажмите <span class="keyseq"><span class="key"><kbd>Ctrl</kbd></span>+<span class="key"><kbd>L</kbd></span></span>, затем затем введите <span class="input">computer:///</span> и нажмите <span class="key"><kbd>Enter</kbd></span>. Если устройство чтения карт памяти настроено правильно, оно должно отображаться как диск, если карта памяти не вставлена, или как сама карта, если карта вставлена.</p></li>
<li class="steps"><p class="p">Если вы видите устройство чтения, но не карту памяти, проблема может быть в самой карте. Попробуйте вставить другую карту памяти или проверьте эту карту памяти на другом устройстве чтения, если есть такая возможность.</p></li>
</ol></div></div></div>
<p class="p">Если карты или диски в локации <span class="gui">Компьютер</span> не отображаются, возможно, что ваше устройство чтения не работает в Linux из-за проблем с драйверами. Это более вероятно, если устройство чтения внутреннее (расположено внутри, а не снаружи компьютера). Лучшим решением будет напрямую подключить ваше устройство (фотокамеру, сотовый телефон и т. д.) к порту USB на компьютере. Существуют также внешние, подключаемые через USB устройства чтения карт памяти, которые поддерживаются в Linux намного лучше.</p>
</div>
